...
class TagsPgDAO implements TagsDAO {
    ...
    public function acomodarTodos($arrayExit) {
        ...
        foreach ($arrayExit as $cate) {
            $objTagsEntity = new Tag();
            $objTagsEntity->setIdTags($cate["idtags"]);
            $objTagsEntity->setTags($cate["tags"]);
            $objTagsEntity->setDescripcionTags($cate["descripciontags"]);
            $allCate[] = $objTagsEntity;
        }
        return $allCate;
    }
